...
DSH Desktop 是 DeepSeek Harness 的桌面客户端。当前源码版本为 1.3.1；桌面安装包本身不内置完整后端。
...
桌面端默认使用 3080 端口：若该端口已有 DSH Web 服务则直接复用（与本机浏览器共享同一实例和插件）；服务存在但桌面会话未认证时，会弹出登录引导，从启动该服务的终端复制 `dsh web:` 打印的登录链接粘贴即可完成认证；未检测到服务时由桌面端在该端口启动。配置了其他端口时直接使用该端口。已有服务返回 401 表示当前会话未认证，不代表服务未运行；无法复用的占用端口不会被强制接管。
...
预览和正式安装包的构建流程见 [用户指南](docs/user-guide.md)。v1.3.1 的源码修改不代表已经发布新的 Setup 安装包。
